...
const EmailValidation: React.FC<EmailValidationProps> = ({
    ...
    const handleInputChange = (index: number, value: string) => {
        if (/^\d*$/.test(value)) { // Only allow numbers
            ...
            // Si el valor tiene más de un carácter (pegar código)
            if (value.length > 1) {
                const pastedCode = value.split('').slice(0, codeLength - index);
                ...
                // Mover el foco al último campo llenado
                const lastFilledIndex = Math.min(index + pastedCode.length - 1, codeLength - 1);
    ...
    const handlePaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
        ...
        const numbersOnly = pastedData.replace(/\D/g, ''); // Eliminar todo lo que no sean dígitos
        
        if (numbersOnly) {
            // Tomar solo los primeros dígitos que necesitamos
            const digitsToPaste = numbersOnly.split('').slice(0, codeLength);
            ...
            // Mover el foco al último campo llenado
            const lastFilledIndex = Math.min(digitsToPaste.length - 1, codeLength - 1);
            inputRefs.current[lastFilledIndex]?.focus();
        }
    };
