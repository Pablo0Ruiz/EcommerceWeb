...
export default function DeliveryAddressPage() {
  ...
  const handleSave = async (updatedAddress: Address) => {
    ...
    await updateProfile({ address: [updatedAddress] });
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <AddressManager enableEdit={true}>
        {({ addresses, loading, error, selectedAddress, handleAddressSelect, handleSetDefault, refetch }) => {
          if (loading) return <div className="container mx-auto px-4 py-8 text-center"><p>Cargando direcciones...</p></div>;
          if (error) return (
          ...
          return (
            <div className="max-w-6xl mx-auto px-4 py-6">
              <h1 className="text-2xl font-bold text-gray-900 mb-6 text-center">Dirección de entrega</h1>
              ...
              <div className="flex flex-wrap justify-center gap-6">
                ...
                {addresses.map((addr) => (
                  <div
                    ...
                    className={`border rounded-lg p-6 w-72 h-[336px] relative shadow-sm hover:shadow-md transition-shadow bg-white flex flex-col cursor-pointer ${selectedAddress?.nombre === addr.nombre ? "border-green-600 border-2" : "border-gray-200"
                      }`}
